...
contract("Election", function(accounts){
  ...
  it("it initializes the candidates with the correct values", function(){
    return Election.deployed().then(function(instance){
      ...
      assert.equal(candidate[1], "Mike", "contains the correct name");
      assert.equal(candidate[2], 0, "contains the correct votes count");
  ...
  it("throw an exception for invalid candidates", function(){
    return Election.deployed().then(function(instance){
      ...
      assert.equal(voteCount, 0, "Mike did not receive any votes");
      return electionInstance.candidates(3);
  ...
  it("throws an exception for double voting", function(){
    return Election.deployed().then(function(instance){
      ...
      var voteCount = candidate2[2];
      assert.equal(voteCount, 1, "Mike did not receive any votes");
      return electionInstance.candidates(3);
    }).then(function(candidate3){
      var voteCount = candidate3[2];
      assert.equal(voteCount, 0, "Elena did not receive any votes");
